...
export class Analyzer {
  ...
  async analyze(params: {
    ...
    const [history, news, klines, etfHistory] = await Promise.allSettled([
      ...
      // SoSoValue free-tier only allows 1d klines; we use those for ATR
      this.sosoClient.getCurrencyKlines(BTC_CURRENCY_ID, { interval: '1d', limit: 14 }),
    ...
    // ── AI conviction ────────────────────────────────────────────────────────
    const decision = await this.convictionEngine.analyze(surprise, market, eventId);
