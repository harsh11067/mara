...
import { Swords, ShieldCheck, HelpCircle, TrendingDown } from 'lucide-react';
import { AccountMenu } from '@/components/AccountMenu';
import { openOnboarding } from '@/components/Onboarding';
import { edgeApi, type EdgeReport, type EdgeStrategyMetrics } from '@/lib/api';
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div>
                  <div className="font-mono text-[10px] tracking-[0.3em] text-amber uppercase mb-4 flex items-center gap-2">
                ...
                <div className="font-mono text-[10px] tracking-widest uppercase text-muted shrink-0">
            ...
            {/* Scoreboard */}
            <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-px bg-foreground/10 border border-foreground/10 mb-12">
              {strategies.map(({ m, hero }) => (
                <div key={m.label} className={`bg-background p-7 ${hero ? 'relative' : ''}`}>
                  ...
                  <div className={`font-mono text-[10px] tracking-[0.25em] uppercase mb-5 ${hero ? 'text-amber' : 'text-muted'}`}>
                    {m.label}
                  </div>
                  <div className={`font-mono text-4xl font-light mb-6 ${m.totalReturnPct >= 0 ? 'text-olive' : 'text-coral'}`}>
                    {pct(m.totalReturnPct)}
                  </div>
                  <div className="space-y-2.5 font-mono text-[10px] tracking-widest uppercase">
                    {([
            ...
            <div className="grid lg:grid-cols-[1fr_420px] gap-8 mb-12">
              ...
              <div className="border border-glass-border bg-background/60 backdrop-blur-xl p-8">
                <div className="font-mono text-[10px] tracking-[0.3em] text-muted uppercase mb-6">
                ...
                <div className="flex flex-wrap gap-x-8 gap-y-2 mt-6 font-mono text-[9px] tracking-widest uppercase">
                  {LINES.map((l) => (
              ...
              <div className="space-y-8">
                <div className="mara-glass p-6">
                  <div className="font-mono text-[10px] tracking-[0.3em] text-muted uppercase mb-5">
                    Regime by regime · honest column included
                  </div>
                  <div className="space-y-3">
                    {report.perRegime.map((r) => (
                      <div key={r.regime} className="flex items-center justify-between gap-3 font-mono text-[10px] tracking-widest uppercase border-b border-glass-border/60 pb-3 last:border-0">
                ...
                <div className="mara-glass p-6">
                  <div className="font-mono text-[10px] tracking-[0.3em] text-muted uppercase mb-4">
                    Monte Carlo · {report.monteCarlo.paths} bootstrap paths
                  </div>
                  <div className="flex gap-10 font-mono">
                    <div>
                      <div className="text-[9px] text-muted tracking-[0.3em] uppercase mb-1">VaR 95</div>
                      <div className="text-2xl font-light text-foreground">{pct(report.monteCarlo.var95Pct)}</div>
                    </div>
                    <div>
                      <div className="text-[9px] text-muted tracking-[0.3em] uppercase mb-1">CVaR 95</div>
            ...
            {/* Stand-down ledger */}
            <div className="border border-glass-border bg-background/60 backdrop-blur-xl p-8 mb-12">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="font-mono text-[10px] text-muted tracking-widest uppercase">{report.standDowns.length} refusals</div>
              </div>
              <div className="max-h-80 overflow-y-auto pr-2 space-y-0">
                {report.standDowns.map((s, i) => (
                  <div key={`${s.date}-${s.eventType}-${i}`} className="grid grid-cols-[90px_90px_1fr_110px] gap-4 items-baseline py-2.5 border-b border-glass-border/50 last:border-0 font-mono text-[10px]">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="border border-glass-border p-6 font-sans text-[11px] text-muted leading-relaxed">
                <span className="text-foreground font-mono text-[9px] tracking-[0.3em] uppercase block mb-2">Method</span>
                {report.method}
              </div>
              <div className="border border-glass-border p-6 font-sans text-[11px] text-muted leading-relaxed">
                <span className="text-foreground font-mono text-[9px] tracking-[0.3em] uppercase block mb-2">Caveats — read them</span>
